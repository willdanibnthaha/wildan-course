      ...
      <div className="bg-white w-full max-w-md rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl animate-in slide-in-from-bottom-10 duration-300" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200 mb-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center text-blue-600"><CreditCard className="w-5 h-5"/></div>
            <div>
          ...
          {/* NOMOR REKENING (BAGIAN PENTING) */}
          <div className="bg-white p-3 rounded-xl border border-slate-200 flex items-center justify-between mb-3 shadow-sm">
            <div>
              {/* PERHATIKAN: TULISAN DI BAWAH INI ADALAH "NOMOR REKENING" */}
              <p className="text-[10px] text-slate-400 uppercase tracking-wider font-bold mb-1">Nomor Rekening</p>
              <p className="font-mono text-xl font-bold text-slate-800 tracking-wide">{paymentInfo.accountNumber}</p>
            </div>
            <button onClick={copyToClipboard} className="p-2 bg-slate-50 hover:bg-blue-50 rounded-lg text-slate-400 hover:text-blue-600 transition-colors">
        ...
        <div className="border-t border-dashed border-slate-200 py-4 mb-2">
          <div className="flex justify-between text-sm items-center mb-4">
            ...
            <span className="font-bold text-slate-900 text-2xl">{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 }).format(plan.price)}</span>
          </div>
          <button onClick={handleWhatsAppConfirm} className="w-full py-4 rounded-xl bg-slate-900 text-white font-bold hover:bg-slate-800 flex items-center justify-center gap-2 transition-all active:scale-[0.98] shadow-lg shadow-slate-900/20">
